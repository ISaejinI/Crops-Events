...
class ThemeSetup
{
    ...
    public function filterEvent($query, $block)
    {
        if (isset($query['post_type']) && $query['post_type'] === 'event') {
            $current_date = date('Y-m-d');

            $query['meta_key'] = 'event_global_informations_event_begin_date';
            $query['orderby'] = 'meta_value';
            $query['order'] = 'ASC';

            $query['meta_query'] = [
                [
                    'key' => 'event_global_informations_event_begin_date',
                    'value' => $current_date,
                    'compare' => '>=',
                    'type' => 'DATE',
                ],
    ...
    public function register()
    {
        ...
        add_filter('query_loop_block_query_vars', [$this, 'filterEvent'], 10, 2);
    }
}